...
public class UnignoreCommand implements CommandExecutor {
    ...
    public UnignoreCommand(NekoChat plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (args.length != 1) {
            player.sendMessage(plugin.getMiniMessage(player, "unignore-usage"));
            return true;
        }
        ...
        if (!plugin.getIgnoreManager().isIgnoring(player.getUniqueId(), target.getUniqueId())) {
            player.sendMessage(plugin.getMiniMessage(player, "unignore-not-found")
        ...
        player.sendMessage(plugin.getMiniMessage(player, "unignore-removed")
                .replaceText(b -> b.matchLiteral("<player>").replacement(target.getName())));
        return true;
    }
}